...
class Line extends AnimatedWidget {
  ...
  const Line({Key key, this.currentNote, this.lineNumber, this.onTileTap, Animation<double> animation}): super(key: key, listenable: animation);
  ...
  Widget build(BuildContext context) {
    ...
    List<Note> thisLineNotes = currentNote.where((note) =>  note.line == lineNumber).toList();
    ...
    List<Widget> tiles = thisLineNotes.map((note){
      ...
      return Transform.translate(offset: Offset(0,offset),
      child: Tile(
        height: tileHeight,
        state: note.state,
