...
class GamePlayOnlineState extends GamePlayState<GamePlayOnline> {
  ...
  List<String> listRanks = ['1st', '2nd', '3rd', '4th'];
  ...
  StreamSubscription subscribePointChanges(DatabaseReference refRoom) {
    return refRoom.onValue.listen((event) {
  ...
  void onTap(Note note) {
    ...
    refRoom
        .child(roomName)
        .update({currentUsernameKey + "Points": super.points});
  }
  ...
  Future<String> calculateFinalRank() async {
    ...
    if (snapshot != null) {
      ...
      // sort userPoints
      listUserPoints.sort((a, b) => b.points.compareTo(a.points));
    }
  ...
  List<TableRow> makeTableRows() {
    ...
    for (var i = 0; i < listUserPoints.length; i++) {
      ...
      rows.add(TableRow(children: [
        ...
        Text(
          '${listUserPoints[i].username}',
          style: style,
          textAlign: TextAlign.center,
        ),
        Text(
          '${listUserPoints[i].points}',
          style: style,
          textAlign: TextAlign.center,
        ),
      ]));
  ...
  void showFinishDialog({String status}) async {
    calculateFinalRank().then((value) {
      ...
      Table tableRanks = Table(border: TableBorder(), children: rows);
      ...
      showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            title: Text("Results"),
            content: SingleChildScrollView(child: tableRanks),
            actions: <Widget>[
              FlatButton(
                onPressed: () {
